...
class MessageField extends React.Component {
  ...
  render() {
    ...
    if (this.state.filteredMessages.length && !this.state.reversed) {
      messageField = (<ul className='MessageField'>{this.state.filteredMessages.map(m => <li className='message' key= {m.key}>{m.createdAt}{m.user.displayName.split(' ').shift()} <br/> {m.content }</li>).reverse()}</ul>);
    } else if (this.state.filteredMessages.length){
      messageField = (<ul className='MessageField'>{this.state.filteredMessages.map(m => <li className='message' key= {m.key}>{m.createdAt}{m.user.displayName.split(' ').shift()} <br/> {m.content }</li>)}</ul>);
    } else if (this.state.userInput !== '') {
      messageField = '';
    } else if (!this.state.reversed) {
      messageField = (<ul className='MessageField'>{this.props.messages.map(m => <li className='message' key= {m.key}>{m.createdAt}{m.user.displayName.split(' ').shift()} <br/> {m.content }</li>).reverse()}</ul>);
    } else {
      messageField = (<ul className='MessageField'>{this.props.messages.map(m => <li className='message' key= {m.key}>{m.createdAt}{m.user.displayName.split(' ').shift()} <br/> {m.content }</li>)}</ul>);
    }
