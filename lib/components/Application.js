  ...
  render() {
    ...
    return (
      <div className="Application">
        { user ? <p> Logged in as {user.displayName} ({user.email}) </p> : <button id='LogInBtn' className='Button' onClick={() => signIn()}>Sign In</button> }
        <MessageField getMessage={this.getMessage.bind(this)} messages={this.state.messages}/>
        { !user ? <p>Hello </p> : <button className='Button' id='LogOutBtn' onClick={() => signOut()}>Sign Out</button> }
